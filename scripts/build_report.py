...
DATE_NAV = r"""
<!-- date-nav: 헤더 우측 날짜 내비게이션 (◀ 날짜/캘린더 ▶) -->
<style>
.dn-inline{display:inline-flex;align-items:center;gap:8px}
#dnPrev,#dnNext{background:rgba(255,255,255,.06);border:1px solid var(--line,#252b3d);color:var(--ink,#f1f3f8);width:28px;height:28px;border-radius:8px;font-size:11px;cursor:pointer;transition:.15s;padding:0}
#dnPrev:hover,#dnNext:hover{background:rgba(255,255,255,.12);border-color:var(--blue,#3b82f6)}
.dn-date{position:relative;display:inline-flex;align-items:center;gap:6px;cursor:pointer;background:rgba(255,255,255,.04);border:1px solid var(--line,#252b3d);border-radius:8px;padding:3px 12px;transition:.15s}
.dn-date:hover{border-color:var(--blue,#3b82f6)}
...
(function(){
  ...
  function init(){
    var el=document.querySelector('.head .meta .date')||document.querySelector('.head .date');
    var cur=null;
    ...
    if(!cur)return;
    var d=parse(cur);
    var label=cur+' ('+WD[d.getDay()]+')';
    var inner='<button id="dnPrev" title="이전 날짜">&#9664;</button>'+
      '<span class="dn-date"><span class="dn-cal">&#128197;</span><span id="dnLabel">'+label+'</span>'+
      '<input type="date" id="dnPick" min="'+MIN+'"></span>'+
      '<button id="dnNext" title="다음 날짜">&#9654;</button>';
    var host;
    if(el){
      el.classList.add('dn-inline');
      el.innerHTML=inner;
      host=el;
    }else{
      host=document.createElement('div');host.id='dateNav';host.innerHTML=inner;
    ...
    host.querySelector('.dn-date').onclick=function(){try{pick.showPicker();}catch(e){}};
  }
...
"""
new_src = new_src.replace("</body>", DATE_NAV + '\n<script src="period-agg.js" defer></script>\n</body>', 1)
